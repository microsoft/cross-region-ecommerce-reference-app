...
if [ -z $HOST ]; then
  echo "No host provided. Assuming the test runs locally against self-hosted datastore. Defaulting to 'localhost:8080'"
  HOST="localhost:8080"
fi
...
# Start and set auto-cleanup for the Docker-Compose environment, if not disabled
if [[ -z $NO_AUTO_START && $HOST =~ ^localhost ]]; then
    echo "Creating Docker Compose test environment..."

    # Run Docker Compose environment cleanup regardless of the exit status of the script
    trap 'docker-compose -f docker-compose.yaml down --remove-orphans' EXIT

    ! echo SA_PASSWORD=$(eval "< /dev/urandom tr -dc _A-Z-a-z-0-9 | head -c12") > .env
